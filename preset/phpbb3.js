...
(function ($) {
	...
	$.fn.insertAttach = function (id, alt, isimg) {
		...
		var data = (isimg === true) ? this.data("wbb").getCodeByCommand("attach", {
			"id": id,
			"num": num,
			"alt": alt
		}) : this.data("wbb").getCodeByCommand("attach", {"id": id, "num": num, "altfile": alt});
		...
		this.$modal.find("#imguploader").dragfileupload({
			...
			success: $.proxy(function (data) {
				...
				if (data && data.status == 1) {
					...
					var datastr = (data.isimg === true) ? this.getCodeByCommand("attach", {
						"id": data.id,
						"num": num,
						"alt": data.alt
					}) : this.getCodeByCommand("attach", {"id": data.id, "num": num, "altfile": data.alt});
